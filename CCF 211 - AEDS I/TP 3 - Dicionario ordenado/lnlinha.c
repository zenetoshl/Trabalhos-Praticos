...
/* Update - TP3 */
/* Copiar Lista de Números de Linha
    Copia uma Lista de Números de Linha para outra, sobrescrevendo seu conteúdo. */
void LNLCopiar(TLNLinha *para,TLNLinha *de)
{
    TLNLinhaCel *atual = de->cabeca->proximo;
    LNLEsvaziar(para);
    LNLCriarVazia(para);

    while (atual!=NULL)
    {
        LNLInserir(para,atual->linha);
        atual = atual->proximo;
    }
